...
- 新后端 `@deepseek-ai/dsh-terminal-bash-human`（类型 `bash-human`）在 POSIX 上服务人类终端。它镜像 `terminal-pwsh`：原始 UTF-8 透传、不净化、`TERM=xterm-256color`、以交互方式（`-i`，从而由 shell 的 rc 文件载入别名、`LS_COLORS` 与提示符）运行用户的登录 shell（`$SHELL`，macOS 上回退到 `/bin/zsh`，其余回退到 `/bin/bash`）、无受控提示符，`startSend`/`signal` 拒绝模型侧就绪契约。关闭会终止整棵进程树。
