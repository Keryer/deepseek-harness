...
- A new backend `@deepseek-ai/dsh-terminal-bash-human` (type `bash-human`) serves the human terminal on POSIX. It mirrors `terminal-pwsh`: raw UTF-8 passthrough with no sanitizer, `TERM=xterm-256color`, the user's login shell (`$SHELL`, falling back to `/bin/zsh` on macOS and `/bin/bash` elsewhere) run interactively (`-i`, so the shell's rc file sources aliases, `LS_COLORS`, and the prompt), no controlled prompt, and `startSend`/`signal` refuse the model-facing readiness contract. Close terminates the whole tree.
