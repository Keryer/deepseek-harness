...
import { API_PATH, HOST_EVENTS_PATH, MUX_EVENTS_PATH, TERMINAL_EVENTS_PATH } from './api-path.ts'
...
export { API_PATH, HOST_EVENTS_PATH, MUX_EVENTS_PATH, TERMINAL_EVENTS_PATH } from './api-path.ts'
...
export function apply(ctx: Context, config?: ConnectionConfig): void {
  ...
  const fetchHandler = connection.createSharedFetchHandler(API_PATH, {
    async fetch(request) {
      ...
      if (request.method === 'GET' && (pathname === MUX_EVENTS_PATH || pathname === HOST_EVENTS_PATH || pathname === TERMINAL_EVENTS_PATH)) {
  ...
  ctx.inject(['apiProxy'], (apiCtx) => {
    ...
    registerDownlink(TERMINAL_EVENTS_PATH, (req, socket, head) => { downlinks.handleTerminal(req, socket, head) })
  })
}
