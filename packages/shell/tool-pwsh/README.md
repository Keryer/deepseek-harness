...
- **No persistent shell or PTY** — every call starts a fresh `pwsh -Command`; the PTY backends are Linux/macOS-only today, and a Windows ConPTY persistent shell is roadmap work. The human embedded terminal now has a Windows PTY backend (`dsh-terminal-pwsh`), but it is human-terminal-only and does not serve the model-facing persistent-shell tool.
