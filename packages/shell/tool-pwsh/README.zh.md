...
- **无持久 shell 或 PTY** — 每次调用都启动全新的 `pwsh -Command`；PTY 后端目前仅限 Linux/macOS，Windows ConPTY 持久 shell 属于路线图工作。人类内嵌终端现已拥有 Windows PTY 后端（`dsh-terminal-pwsh`），但它仅面向人类终端，不服务于面向模型的持久 shell 工具。
