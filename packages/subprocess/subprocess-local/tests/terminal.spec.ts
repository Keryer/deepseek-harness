...
describe('LocalTerminalHandle', () => {
  ...
  it('resizes the terminal grid and rejects after exit', async () => {
    const pty = new FakePty()
    const inspector = new FakeInspector()
    const handle = new LocalTerminalHandle(pty.asPty(), inspector, 10)
  ...
  it('takes the whole tree through taskkill before shell escalation on Windows', async () => {
    const pty = new FakePty()
    const inspector = new FakeInspector()
    const taskkill = vi.fn()
    const handle = new LocalTerminalHandle(pty.asPty(), inspector, 10, 'win32', taskkill)

    await handle.terminate()

    expect(taskkill).toHaveBeenCalledWith(pty.pid)
    expect(pty.kills).toEqual(['SIGTERM'])
  })

  it('force-kills the tree through taskkill during synchronous host exit on Windows', () => {
    const pty = new FakePty()
    const inspector = new FakeInspector()
    const taskkill = vi.fn()
    const handle = new LocalTerminalHandle(pty.asPty(), inspector, 10, 'win32', taskkill)
