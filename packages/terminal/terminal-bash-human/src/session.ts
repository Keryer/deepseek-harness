 ...
 * colors and cursor control exactly as the shell emits them; like the
 * PowerShell sibling (`terminal-pwsh`), this session serves no model-facing
 * line-oriented consumer, so `startSend` and `signal` refuse rather than
 * half-support the POSIX readiness contract.
 */
...
export class BashHumanPtySession implements TerminalBackendSession {
  motd = ''
  readonly pid: number
  ...
  startSend(_request: TerminalSendRequest): TerminalSendOperation {
    throw new Error('terminal-bash-human: line-oriented send requires the model-facing bash PTY backend')
  }
  ...
  signal(_signal: TerminalSignal): Promise<TerminalSignalResult> {
    return Promise.reject(new Error('terminal-bash-human: foreground signalling is unsupported; write \\x03 for Ctrl+C'))
  }
