...
import { BashHumanPtySession } from '@deepseek-ai/dsh-terminal-bash-human/src/session.ts'

function config(overrides: Partial<ResolvedConfig> = {}): ResolvedConfig {
  return {
    backendType: 'bash-human', shellPath: '/bin/bash', shellArgs: ['--noprofile', '--norc', '-i'],
    rows: 24, cols: 80, scrollbackLines: 10, scrollbackMaxBytes: 1000, maxReadBytes: 200,
...
describe('BashHumanPtySession', () => {
  it('forwards raw writes and resizes and streams raw output including ANSI', async () => {
    const fake = fakeHandle()
    const session = new BashHumanPtySession(fake.handle, config())
    const chunks: string[] = []
  ...
  it('captures the shell greeting as motd during initialize', async () => {
    ...
    expect(session.motd).toBe('user@host:~$ ')
    await session.close('done')
  })
  ...
  it('bounds the startup wait and reports a shell that exited during startup', async () => {
    ...
    const silentSession = new BashHumanPtySession(silent.handle, fast)
    await silentSession.initialize()
    expect(silentSession.motd).toBe('')
    await silentSession.close('done')

    const exited = fakeHandle()
    const exitedSession = new BashHumanPtySession(exited.handle, fast)
    exited.exit(1)
  ...
  it('bounds scrollback by bytes and lines and reports truncation', () => {
    ...
    const fake = fakeHandle()
    const session = new BashHumanPtySession(fake.handle, small)
  ...
  it('clears the cached close promise and rethrows a cleanup failure', async () => {
    ...
    const session = new BashHumanPtySession(handle, config())
